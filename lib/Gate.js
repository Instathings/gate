...
class Gate extends EventEmitter {
  ...
  start() {
    ...
    const { discoverBaseTopic, controlBaseTopic } = this.discoverBaseTopic;
    this.once('gate_connected', onGateConnected.bind(this));
    this.device.on('connect', onConnect.bind(this, discoverBaseTopic, controlBaseTopic));
    this.device.on('error', debug);
    this.device.on('disconnect', debug);
    this.device.on('error', debug);
    this.device.on('reconnect', debug);
    this.device.on('message', onMessage.bind(this)); // TODO
  }
}
