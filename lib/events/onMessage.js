...
module.exports = function onMessage(topic, payload) {
  ...
  switch (topic) {
    case `${this.discoverBaseTopic}/device/post`: {
      strategy = new PairSubdeviceStrategy(topic, message, this.device, this.knownDevices, this.discoverBaseTopic, this);
      break;
    }

    case `${this.discoverBaseTopic}/protocol/post`: {
      strategy = new InstallStrategy(topic, message, this.device, this.knownDevices, this.discoverBaseTopic, this);
      break;
    }

    case `${this.discoverBaseTopic}/software/update`: {
      strategy = new UpdateSoftwareStrategy(topic, message, this.device, this.knownDevices, this.discoverBaseTopic, this);
      return strategy.execute((err) => {
        if (err) {
          logError(err);
        }
        /**
         * Always notify version
         */
    ...
    case `${this.discoverBaseTopic}/protocol/update`: {
      strategy = new UpdateProtocolStrategy(topic, message, this.device, this.knownDevices, this.discoverBaseTopic, this);
      return strategy.execute((err) => {
        /**
