  ...
  execute() {
    ...
    return async.auto({
      ...
      listAfter: ['install', (results, autoCb) => {
        const { listBefore } = results;
        const idsBefore = []; const idsAfter = []
        listBefore.map((element) => { idsBefore.push(element.Names[0].replace('/', '')); });
        docker.listContainers((err, allContainers) => {
          if (err) {
            return autoCb(err);
          }
          allContainers.map((element) => { idsAfter.push(element.Names[0].replace('/', '')); });
          const newContainersNames = idsAfter.filter(x => !idsBefore.includes(x));
      ...
      versions: ['listAfter', (results, autoCb) => {
        const containers = results.listAfter.allContainers;
      ...
      logs: ['publish', (results, autoCb) => {
        ...
        const { newContainersNames } = results.listAfter;
        console.log('NEW', newContainersNames);
